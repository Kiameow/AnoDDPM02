...
class OPMEDDataset(Dataset):
    def __init__(self, root_dir, modality='FLAIR', train=True, img_size=(256, 256)):
        ...
        if train:
            self.image_dir = os.path.join(root_dir, 'train', f'hc_{modality}')
            self.mask_dir = None  # No masks for healthy controls
        else:
            self.image_dir = os.path.join(root_dir, 'test', f'{modality}_test')
            self.mask_dir = os.path.join(root_dir, 'test', 'mask_test')
...
if __name__ == "__main__":
    ...
    #         )
    d_set = OPMEDDataset('./dataset', train=False)
    loader = init_dataset_loader(d_set, {"Batch_Size": 1})
